---
title: Textual Criticism
date: Spring 2026
bibliography: bibliographies/engl413.yaml
---

[zotero-library]: https://www.zotero.org/irc7/collections/XAL6EQTA
[gh-releases]: https://github.com/icornelius/zg-syllabi/releases
[Sakai Resources]: https://sakai.luc.edu/portal/site/ENGL_413_001_5454_1262/tool/31ad6876-12b7-42c1-9dc6-24d36f3e1243

\newpage

# Basic information

## Registrar details

- Course number: ENGL 413-001 (5454)
- Meeting time: MW 4:15PM - 5:30PM
- Location: Cuneo Hall - Room 212

## How to contact me
``` {.include}
partials/course_id-0004-20230107090154.md
```

# Course summary and outcomes
``` {.include}
partials/course_desc-0011-20251030151040.md
```

Learning outcome:

``` {.include}
partials/outcome-0006-20260110110008.md
```

# Materials

## Basic orientations
``` {.include}
partials/texts-0011-20260109213812.md
```

## Required readings
``` {.include}
partials/texts-0007-20250817104713.md
```

# Schedule

``` {.include}
partials/schedule-0005-20240110143330.md
schedules/engl413-2026-01.md
```

# Grading and assignments
## Summary of grade components
``` {.include}
partials/assessment-0013-20260109221304.md
```

## Participation and presentations
``` {.include}
partials/assignments-0060-20260109223139.md
```

## Written work

### General instructions
``` {.include}
partials/assignments-0062-20260110102931.md
```

### Overview of assignments
``` {.include}
partials/assignments-0061-20260109224711.md
```

## Grade schema
``` {.include}
partials/assessment-0002-20220818165633.md
```

# Policies

## Communication
``` {.include}
partials/policies-0007-20220818163137.md
```

## Attendance
If you must miss a class, or if you must arrive late or leave early, inform me in advance by email.

## Academic integrity
``` {.include}
partials/policies-0020-20260109203910.md
```

## Equity and compliance

### Nondiscrimination statement
``` {.include}
partials/policies-0013-20240812164628.md
```

### Religious observances
``` {.include}
partials/policies-0015-20240816165914.md
```

### Reporting obligations for Responsible Campus Partners
``` {.include}
partials/policies-0014-20240812165200.md
```

### Names and pronouns
``` {.include}
partials/policies-0008-20220818163739.md
```

### Accommodations and assistance
``` {.include}
partials/policies-0009-20220818163938.md
```

## Privacy
``` {.include}
partials/policies-0005-20220818161952.md
```

## Statement of intent
By remaining in this course, students agree to accept this syllabus and abide by its policies.
Students will be informed of any changes to the syllabus.

# License and rights

This syllabus is marked [CC0 1.0 Universal](https://creativecommons.org/publicdomain/zero/1.0/).

Students are reminded that many course readings are protected by copyright and should not be shared outside this course without written permission of the copyright holder.

# Version information
Last compiled \today.

[View file history on GitHub][gh-releases]

# Bibliography

These details are available on [Zotero][zotero-library] in machine-readable formats.
